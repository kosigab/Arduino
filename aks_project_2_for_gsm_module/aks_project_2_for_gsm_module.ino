...
void setup()
{
 myservo.attach(13);
  ...
  pinMode (VLVK, INPUT);
  pinMode (SSSMS, INPUT);
  pinMode (vuvgsm, OUTPUT);
  pinMode (vlvgsm, OUTPUT);
  pinMode (relay, OUTPUT);

  digitalWrite (vuvgsm, LOW);
  digitalWrite (relay, LOW);
  digitalWrite (vlvgsm, LOW);
  digitalWrite (VUVK, LOW);
  digitalWrite (VLVK, LOW);
  digitalWrite (SSSMS, LOW);
...
void loop() {
  ...
  //  delay(100);
   ...
   vuvk = digitalRead(VUVK);
  vlvk = digitalRead(VLVK);
  sssms = digitalRead(SSSMS);
...
void sendsmslockedDuetoinvalidcode()
{
  // delay (1000);
  Serial.println("Sending sms...");
  gprs.sendSMS("+2348106137871", "Your vehicle was just locked due to the input of invalid codes! We strongly recommend that you check the status of your vehicle."); //define phone number and text
  //
  //   serialSIM800.write("AT+CMGF=1\r\n");
  //   delay(1000);
  //
